...
data {
  ...
  int<lower=1, upper=6> option1[num_subjects, num_trials];
  int<lower=1, upper=6> option2[num_subjects, num_trials];
  int<lower=1, upper=2> choice[num_subjects, num_trials];
  real outcome_pos[num_subjects, num_trials];
  real outcome_neg[num_subjects, num_trials];
}
...
parameters {
  ...
  vector[num_subjects] learnrate_pos_pr;
  vector[num_subjects] learnrate_neg_pr;
  vector[num_subjects] discount_pos_pr;
  vector[num_subjects] discount_neg_pr;
...
transformed parameters {
  ...
  vector<lower=0, upper=20>[num_subjects] tau; //why 20? (perhaps narrowed to 20 bc no normal distribution)
  
  // take a draw from standard normal and shift the means to 
...
model {
  ...
  for (i in 1:num_subjects) {
    ...
    for (t in 1:num_trials) {
      ...
      // only factors in to the updating when it's the 3rd episode. (episodes: option selection, action, outcome; so below we are getting RPE and updated
...
generated quantities {
  ...
  // grab population level estimates (the entire data we're feeding in must be these parameters)
